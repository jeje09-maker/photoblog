...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // 諛붿씠?몃? ?щ엺???쎄린 ?ъ슫 臾몄옄?대줈 蹂??  function formatFileSize(bytes) {
  ...
  btnExtractKeywords.addEventListener('click', async () => {
    ...
    const parisKeywords = ['?뚮━?ы뻾', '?먰렆?묒빞寃?, '?꾨옉?ㅺ컧?깆뿬??, '?먰렆?묒뭅??, '?좊읇?먯쑀?ы뻾'];
    const orangeryKeywords = ['?뚮━?ㅻ옉二쇰━', '?앸Ъ?먯뭅?섏텛泥?, '?뚯＜?쒕씪?대툕肄붿뒪', '?뚯＜移댄럹異붿쿇', '?⑥떎移댄럹'];
    const genericKeywords = ['?쇱긽釉붾줈洹?, '?곗씠?몄퐫?ㅼ텛泥?, '媛먯꽦?ㅽ넗由?, '二쇰쭚?섎뱾??, '遺꾩쐞湲곗쥕?怨?];
  ...
  presetParis.addEventListener('click', async () => {
    ...
    keywordsList = ['?뚮━?ы뻾', '?먰렆?묒빞寃?, '?꾨옉?ㅺ컧??];
    renderKeywordTags();
  ...
  presetOrangery.addEventListener('click', async () => {
    ...
    keywordsList = ['?뚯＜?ㅻ옉二쇰━', '?앸Ъ?먯뭅?섏텛泥?, '?뚯＜?곗씠?몄퐫??];
    renderKeywordTags();
...
異붽? 臾섏궗: ${inputDesc.value}` },
  ...
  async function generateBlogWithGemini() {
    ...
    switch (currentCategory) {
      case 'restaurant':
        categoryToneInstruction = '留쏆쭛/?몃뱶 遺꾩빞: ?쒓컖怨?誘멸컖???뗭슦??援곗묠 ?꾨뒗 臾섏궗瑜?媛????   const humanizeRule = `[???대㉫ ?섎Ⅴ?뚮굹 ?꾩닔]: ?몄쐞?곸씤 AI 留먰닾(~寃껋쑝濡?異붿젙?⑸땲?? ~?⑸땲????湲곌퀎???댄닾)瑜??꾨꼍??踰꾨━?몄슂. ?ㅼ젣 ?щ엺???꾩옣?먯꽌 ?ㅺ컧???듯빐 ?먮? 媛먯젙, ?ㅼ닔?? ??쇱?, 吏꾩쭨 留뚯”?덈뜕 遺遺???'?대㉫ ?섎Ⅴ?뚮굹'瑜?200% ?댁븘?댁뼱 吏꾩쭨 ?щ엺????寃?媛숈? ?앸챸?μ쓣 遺덉뼱?ｌ쑝?몄슂.`;
...
4. ${articleRule}
5. ${humanizeRule}
6. 蹂몃Ц ?댁슜怨?移댄뀒怨좊━???꾨꼍??遺?⑺븯???듭떖 寃???ㅼ썙??10媛쒕? ?ㅼ뒪濡?異붿텧?섍퀬, 蹂몃Ц 以묎컙以묎컙???먯뿰?ㅻ읇寃??뱀뿬?댁뼱 寃???붿쭊 ?몄텧 ?먯닔瑜?洹밸??뷀븯?몄슂.`;옄洹??ъ씤?몃? ?ㅻ챸?섎뒗 ?숆린遺???댁“';
        break;
      case 'finance':
...
<p><strong>Q1. ?먰렆???몄쓣 媛먯긽 踰좎뒪???ㅽ뙚??</strong><br>?ㅼ슂 沅?愿묒옣?먯꽌 ?뺣㈃ 酉곕? 異붿쿇?⑸땲??<br><br><strong>Q2. 怨⑤ぉ 移댄럹 異붿쿇 硫붾돱媛 ?덈굹??</strong><br>?먯뒪?꾨젅?뚯? ?щ（?꾩긽 議고빀??媛뺤텛?댁슂!<br><br><strong>Q3. ?뚮ℓ移섍린 ?덈갑 轅??</strong><br>媛諛⑹쓣 ?욎쑝濡?硫붽퀬, ?ㅻ쭏?명룿???먮ぉ ?ㅽ듃?⑹쓣 ?곌껐?섏꽭??</p>` : ''}`;
          ...
          title = "?뚮━ ?먰렆??紐낅떦 酉?& 二쇰? 怨⑤ぉ 移댄럹 肄붿뒪 珥앹젙由?;
...
<p><strong>Q1. ?몄젣 諛⑸Ц?섎뒗 寃껋씠 遺꾩쐞湲곌? 醫뗫굹??</strong><br>?뉗궡??遺?쒕읇寃??대━弛먮뒗 ??? ?ㅽ썑 ?쒓컙?瑜??곴레 異붿쿇?⑸땲??<br><br><strong>Q2. 諛⑸Ц ???뚯븘???곸씠 ?덈굹??</strong><br>?ъ쑀濡?쾶 怨듦컙???꾨뒔???뚮━? ?뺤랬??吏묒쨷?대낫?쒓린 諛붾엻?덈떎.<br><br><strong>Q3. ?ъ쭊?????섏삤??媛먯꽦 ?ㅽ뙚???</strong><br>?먯뿰愿묒씠 ?댁쭩 ?ㅼ뼱?ㅻ뒗 李쎄? 洹쇱쿂???뚰뭹???덉걯寃?紐⑥뿬?덈뒗 踰쎈㈃??醫뗭뒿?덈떎.</p>` : ''}`;
          ...
          case 'seo':
            title = `[?뺣낫] ${subject} ?붿쭅 ?꾧린 - ?꾩튂, 二쇱감, ?댁슜 轅??珥앹젙由?;
            body = `諛⑸Ц媛?遺꾨뱾???꾪빐 ${dateStr}??吏곸젒 寃쏀뿕??${subject} ?듭떖 ?뺣낫瑜?媛?낆꽦 ?믪? ?뺣━湲濡??꾨떖?⑸땲?? ${photoText}

1. ${subject} 湲곕낯 遺꾩꽍
- 諛⑸Ц ?쇱옄: ${dateStr}
- 二쇱슂 ?ㅼ썙?? ${kwText}
${desc ? `- ?꾩옣 ?ㅼ궗???곸꽭 ?댁슜: ${desc}` : '- ?곸꽭 ?뱀쭠: 源붾걫?섍퀬 ?꾨??곸씤 ?쒖꽕 愿由?}
...
<p><strong>Q1. ?쇱옄 媛湲곗뿉??愿쒖갖? 臾대뱶?멸???</strong><br>?? ?꾨뒔?섍쾶 ??援ъ꽍???먮━ ?↔퀬 梨낆쓣 蹂닿굅???앷컖?섍린?먮룄 李?醫뗫떟?덈떎.<br><br><strong>Q2. 諛섎젮?숇Ъ ?숇컲??媛?ν븷源뚯슂?</strong><br>援ъ뿭?대굹 議곌굔???곕씪 ?곸씠?섎?濡?諛⑸Ц ?꾩뿉 留ㅼ옣 ?좎꽑 ?뺤씤??異붿쿇?댁슂.<br><br><strong>Q3. ?ъ옣?섏씠 異붿쿇?섏떆???쒓렇?덉쿂 轅議고빀??</strong><br>湲곕낯??異⑹떎?섎ŉ ?щ즺 蹂몄뿰??留쏆씠??硫뗭쓣 ?대┛ ??쒖쟻???좏깮??異붿쿇?⑸땲??</p>` : ''}`;
          ...
          case 'formal':
            title = `${subject}?????怨듦컙???뱀꽦 諛??묎컼 留뚯”???곸꽭 蹂닿퀬??;
...
<p><strong>Q1. ?⑥껜 怨좉컼 ?섏슜 ?λ젰??異⑸텇?쒓???</strong><br>?ъ쟾 ?묒쓽瑜?留덉튌 寃쎌슦 異⑸텇???ъ쑀 怨듦컙 諛곗젙??媛?ν븳 洹쒕え?낅땲??<br><br><strong>Q2. ?쎌옄 諛곕젮 ?몄쓽 ?쒖꽕???꾩엯?섏뿀?섏슂?</strong><br>吏꾩엯濡?寃쎌궗 ?쒖꽕 諛??섎━踰좎씠???숈꽑??湲곕낯 援ъ“??諛섏쁺?섏뼱 ?덉뒿?덈떎.<br><br><strong>Q3. ?뺣낫 ?띾뱷???좎슜??怨듭떇 寃쎈줈瑜??쒖븞諛붾엻?덈떎.</strong><br>怨듭떇 ?붿???梨꾨꼸 諛?二쇨린?곸씤 ?덉빟 ?숉뼢???곸떆 援먯감 泥댄겕?댁＜?쒓린 諛붾엻?덈떎.</p>` : ''}`;
          ...
          case 'humorous':
            title = `[寃쎄퀬] 吏媛?猷⑦뙜 ?뱁븯怨???${subject} 李??먯뀡 ?꾧린 ?뗣뀑??;
...
<p><strong>Q1. 二쇱감 怨듦컙? ?ъ쑀濡쒖슫 ?몄씤媛?</strong><br>二쇰쭚 ?ㅽ썑??붾뜲??二쇱감??怨듦컙 ?됰꼮?댁꽌 臾몄젣 ?놁뿀??<br><br><strong>Q2. ?쇱옄 媛湲곗뿉???곹빀?쒓??</strong><br>議곗슜???묒뾽?섍굅??硫??뚮━湲?理쒖쟻?대씪 ?쇨?議깅텇???곴레 沅뚯옣??<br><br><strong>Q3. 異붿쿇 ?ъ씤???섎굹留?以?ㅻ㈃?</strong><br>援ъ꽍援ъ꽍 ?뱀븘?덈뒗 ?뚯냼???ъ씤?몃뱾 ?뺣텇???명븯寃?異⑹쟾?????덉쓬.</p>` : ''}`;
        ...
        switch (currentCategory) {
          case 'restaurant':
            ...
            break;
          case 'cafe':
            title = `[移댄럹] 媛먯꽦 ???붽낵 ?꾨뒔?? ?몄뒪? ?ロ뵆 ${subject} ?ㅻ?????;
            categoryContentHtml = `
    ...
    return { 
      ...
      seoTitles: [
        title, 
        "?ロ뵆?덉씠??媛먯꽦 ?섏튂???붿쭅 ?먮갑湲?, 
        "?ъ쭊?쇰줈 湲곕줉?섎뒗 ?덈???李곕굹???쒓컙", 
        "留롮? ?щ엺?ㅼ씠 ?닿킅?섎뒗 ?댁쑀? 珥앺룊", 
        "?뚭퀬 蹂대㈃ ??留ㅻ젰?곸씤 ?듭떖 泥댄겕由ъ뒪??
      ], 
      seoKeywords: ['媛먯꽦釉붾줈洹?, '?곗씠?몄텛泥?, '?먮쭅?ы뻾', '遺꾩쐞湲곕쭧吏?, '?ъ쭊李띻린醫뗭?怨?, '?ロ뵆?ъ뼱', '?몄깮?룸챸??, '?붿쭅?꾧린', '?대룉?댁궛', '?몃젋?쒗깘??] 
    };
  }
  ...
  function renderGeneratedOutput() {
    ...
    // SEO 寃곌낵 諛뺤뒪 (?쒕ぉ 5媛?& ?ㅼ썙??10媛? ?명꽣?숉떚釉??뚮뜑留?    if (seoResultsCard && seoTitleList && seoKeywordGrid) {
      if ((generatedSeoTitles && generatedSeoTitles.length > 0) || (kwsToUse && kwsToUse.length > 0)) {
        seoResultsCard.classList.remove('hidden');
        
        // 異붿쿇 ?쒕ぉ Top 5 ?뚮뜑留?        seoTitleList.innerHTML = '';
        const titlesToUse = (generatedSeoTitles && generatedSeoTitles.length > 0) 
          ? generatedSeoTitles 
          : [generatedTitle, "媛먯꽦怨??먮쭅??媛?앺븳 ?밸퀎??寃쏀뿕 ?꾧린", "?ъ쭊?쇰줈 湲곕줉?섎뒗 ?뚯쨷??李곕굹???쒓컙", "??媛怨??띠? 理쒓퀬???ロ뵆?덉씠???먮갑湲?, "?뚭퀬 媛硫???醫뗭? ?듭떖 ?ъ씤???뺣━"];
        ...
        // 異붿쿇 ?ㅼ썙??10???뚮뜑留?        seoKeywordGrid.innerHTML = '';
